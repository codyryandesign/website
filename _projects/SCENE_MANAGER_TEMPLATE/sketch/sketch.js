function setup() {
  ...
  const MAIN_SCREEN_MAX_WIDTH = DISPLAY_WIDTH/3;
  const MAIN_SCREEN_MAX_HEIGHT = DISPLAY_HEIGHT/3;

  const NUM_SCREENS = 6;
  const TV_BORDER = 10;
  const TV_WIDTH = (MAIN_SCREEN_MAX_WIDTH/NUM_SCREENS)-TV_BORDER;
  const TV_HEIGHT = (MAIN_SCREEN_MAX_HEIGHT/NUM_SCREENS)-TV_BORDER;

  const SIDE_SCREEN_MAX_WIDTH = DISPLAY_WIDTH/3;
  const SIDE_SCREEN_MAX_HEIGHT = DISPLAY_HEIGHT/3;
  ...
  //This function keeps sketches running in the background
  mgr.showNextScene();
}
...
function keyPressed()
{
    //you can optionally handle the key press at global level...
  switch(key)
  {
    case '1':
  ...
  //...then dispatch via the ScneManager.
